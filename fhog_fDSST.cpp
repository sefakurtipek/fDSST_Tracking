...
// compute fhog_fDSST features
void fhog_fDSST(float* M, float* O, float* H, int h, int w, int binSize,
...
void mGradHist(int nl, mxArray* pl[], int nr, const mxArray* pr[]) {
    ...
    if (useHog == 0) {
        ...
        fhog_fDSST(M, O, H, h, w, binSize, nOrients, softBin, clipHog);
    }
}
...
float* fhog_fDSST(float* I, int height, int width, int channel, int* h, int* w, int* d, int binSize, int nOrients, float clip, bool crop) {
    ...
    *d = nOrients * 3 + 5;
    ...
    fhog_fDSST(M, O, H, height, width, binSize, nOrients, -1, clip);
...
cv::Mat fhog_fDSST(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop) {
    ...
    int h, w, d;
    float* HH = fhog_fDSST(I, HEIGHT, WIDTH, DEPTH, &h, &w, &d, binSize, nOrients, clip, crop);
    float* H = new float[h * w * d];
