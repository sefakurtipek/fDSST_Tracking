...
#include "ffttools_fDSST.hpp"
#include "recttools_fDSST.hpp"
#include "fhog_fDSST.h"
#include "labdata_fDSST.hpp"
...
cv::Point2i FDSSTTracker::detect_scale(cv::Mat image)
{
    ...
    cv::Mat add_temp;
    cv::reduce(FFTTools_fDSST::complexMultiplication(sf_num, xsf), add_temp, 0, cv::REDUCE_SUM);

    // compute the final y
    cv::Mat scale_responsef = FFTTools_fDSST::complexDivisionReal(add_temp, (sf_den + scale_lambda));
    ...
    interp_scale_response = FFTTools_fDSST::real(interp_scale_response);
...
cv::Mat FDSSTTracker::createGaussianPeak(int sizey, int sizex)
{
    ...
    return FFTTools_fDSST::fftd(res);
}
...
cv::Mat FDSSTTracker::getFeatures(const cv::Mat& image, bool inithann, float scale_adjust)
{
    ...
    cv::Mat FeaturesMap;
    cv::Mat z = RectTools_fDSST::subwindow(image, extracted_roi, cv::BORDER_REPLICATE);
    ...
    // HOG features   
    FeaturesMap = fhog_fDSST(z, cell_size);
...
void FDSSTTracker::train_scale(cv::Mat image, bool ini)
{
    ...
    cv::reduce(FFTTools_fDSST::real(new_sf_den), new_sf_den, 0, cv::REDUCE_SUM);
...
cv::Mat FDSSTTracker::get_scale_sample(const cv::Mat& image)
{
    ...
    for (int i = 0; i < n_scales; i++)
    {
        ...
        // Get the subwindow
        cv::Mat im_patch = RectTools_fDSST::extractImage(image, cx, cy, patch_width, patch_height);
        ...
        cv::Mat hogs = fhog_fDSST(im_patch_resized, cell_size);
    ...
    xsf = FFTTools_fDSST::fftd(xsf, 0, 1);
